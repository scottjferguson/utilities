...
namespace Utilities.FileProcessors.XVerify
{
    ...
    public class XVerifyMailingAddressByServiceAddressValidFileProcessor : UtilitiesBase, IProcessor
    {
        ...
        private readonly string _filePath = @"C:\PATaxAffidavitMailingValidAddresses.csv";

        public XVerifyMailingAddressByServiceAddressValidFileProcessor(
...
SET isUseMailingAddressAlt = 1
  ...
  , modifyUser = 'UpdateMailingAddressByServiceAltFromXVerify'
  , modifyTS = CURRENT_TIMESTAMP
WHERE 1 = 1
  AND serviceAddressLine1 = @inputAddress
  AND serviceAddressCity = @inputCity
  AND serviceAddressState = @inputState";

            if (!string.IsNullOrEmpty(inputAddressLine2))
            {
                sql += "  AND serviceAddressLine2 = @inputAddressLine2";
            }
